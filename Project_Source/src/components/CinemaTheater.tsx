...
import { Maximize, Minimize, Play, Pause, SkipForward, SkipBack, Sparkles, Tv, Eye, VolumeX, Volume2, AlertCircle, RefreshCw, Layers, ChevronUp, ChevronDown, ExternalLink, ShieldAlert, Pencil, Trash2, Search } from 'lucide-react';
...
interface CinemaTheaterProps {
  ...
  onNavigatePrevFolder?: () => void;
  allFolders?: Folder[];
  onShowFullFolderView: () => void;
}
            ...
            <div className="absolute top-4 right-4 left-4 flex justify-between items-center pointer-events-none select-none z-20">
              ...
              <div className="px-4 py-2 bg-black/80 backdrop-blur-md border border-neutral-800 rounded-xl flex items-center gap-3">
                <button
                  onClick={onNavigatePrevFolder}
                  disabled={!allFolders || allFolders.length === 0}
                  className="p-1 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 hover:text-cyan-300 rounded transition-all disabled:opacity-30 pointer-events-auto"
                  title="المستودع السابق"
                >
                ...
                <button
                  onClick={onShowFullFolderView}
                  className="text-white font-bold hover:text-cyan-300 transition-colors px-1 py-0.5 rounded pointer-events-auto text-sm"
                  title="عرض المستودع"
                >
                  {currentFolder ? currentFolder.name : '—'}
                </button>
              ...
              <div className="flex gap-2 pointer-events-auto items-center">
                ...
                <div className="px-3 py-2 bg-black/80 backdrop-blur-md border border-neutral-800 rounded-xl flex items-center gap-2">
                  ...
                  <span className="text-white text-xs font-bold">({currentItemIndex + 1}/{currentFolder?.items.length || 0})</span>
                  <button
                    onClick={onNavigateNext}
                    disabled={!currentFolder || currentFolder.items.length === 0}
                    className="p-1 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 hover:text-emerald-300 rounded transition-all disabled:opacity-30"
                ...
                {item && (
                  <>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1.5 px-3 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold text-xs transition-all shadow-lg shadow-blue-900/20"
                      title="بحث في جوجل"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1.5 px-3 py-2.5 bg-red-600 hover:bg-red-500 text-white rounded-xl font-bold text-xs transition-all shadow-lg shadow-red-900/20"
                      title="بحث في ياندكس"
                    >
                      <Search className="w-4 h-4" />
                    </a>
